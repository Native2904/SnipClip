...
**A small popup can confirm every capture** – off by default. Enabled, it briefly shows a short preview (text or a scaled-down screenshot) plus the source program's name in a corner of the screen, then disappears on its own or with one click. Never steals focus. Not a tray-icon balloon or a system toast on purpose – see `[notifications]` below.
...
Off by default (`Enabled=0`). Once on, every capture (text or screenshot) briefly shows a small popup with a short content preview and the source program's name. Deliberately not a tray-icon balloon or a Windows Action Center toast – this plugin has no tray icon by design, and a modern toast needs an app identity/COM registration that's fragile for a plain unpackaged Win32 DLL built with MinGW. Instead it's a small, self-drawn window that never steals focus (`WS_EX_NOACTIVATE`).

- **`DurationMs`** (default 3000) – how long it stays visible before disappearing on its own; a click dismisses it immediately regardless
- **`Corner`** (default `BottomRight`) – `TopLeft`/`TopRight`/`BottomLeft`/`BottomRight`, which corner of the screen's work area it appears in
- **`FontSize`** (default 14, range 6-72) – the source-program line underneath stays about 2pt smaller automatically
- **`IgnoreTheme`** (default 0) – 1 = always a fixed white background/black text, regardless of `[Theme]`/`NoColors=`, for guaranteed readability independent of the rest of the plugin's current look
